...
- Pose skeleton: 2px lines + 5px joint dots in the current stage color at 80% opacity; landmarks below confidence threshold render dashed at 40%.
...
**Toast notifications.** Windows native toasts. Title = observation, body = one concrete fix. Tone: a friendly coach who noticed, never a scold — escalation adds urgency through specificity and duration, not blame. `{side}` interpolates `left`/`right`.
...
| `PoseOverlay` | `landmarks`, `stageColor`, `confidenceThreshold`; absolute SVG over feed |
